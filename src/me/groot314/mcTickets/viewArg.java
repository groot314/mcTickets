...
public class viewArg {
	...
	public void view(Player p, String[] arg) {
		...
		if (args.length == 1) {
			...
			if (player.hasPermission("mctickets.admin.view.all"))
				viewAll();
			else {
			...
			if (player.hasPermission("mctickets.admin.view.open"))
				viewOpen();
			else {
				player.sendMessage(Color.RED
						+ "You dont have permission to that command");
			...
			if (player.hasPermission("mctickets.admin.view.closed"))
				viewClosed();
			else {
				player.sendMessage(Color.RED
						+ "You dont have permission to that command");
			...
			if (player.hasPermission("mctickets.admin.view.taken"))
				viewTaken();
			else {
				player.sendMessage(Color.RED
						+ "You dont have permission to that command");
			...
			if (player.hasPermission("mctickets.admin.view.mine"))
				viewMine();
			else {
				player.sendMessage(Color.RED
						+ "You dont have permission to that command");
			...
			player.sendMessage(ChatColor.DARK_AQUA + "[" + ticketNumber + "]["
					+ SQL.getTicketInfo(ticketNumber, "User") + "]");
			for (int i = 0; i < rNs.size(); i++) {
				int id = rNs.get(i);

				player.sendMessage(userColor(ticketNumber,
						SQL.getReplyInfo(id, "User"))
						+ "["
						+ SQL.getReplyInfo(id, "User")
						+ "]"
						+ ": "
			...
			if (rNs.size() == 5) {
				player.sendMessage(ChatColor.BLUE + "[For 2nd page]:"
						+ ChatColor.AQUA + " /ticket view <Ticket> <Page>");
			}
		} else {// ticket view <#>
	...
	private void viewAll() {
		...
		for (int i = 0; i < tNs.size(); i++) {
			...
			player.sendMessage(ChatColor.BLUE + "[" + String.valueOf(tN) + "]"
					...
					+ SQL.getTicketInfo(tN, "Status") + "]" + ChatColor.BLUE
	...
	private void viewTaken() {
		ArrayList<Integer> tNs = SQL.getTickets(pageNumber, "Assigned",
				player.getName());
	...
	private void viewMine() {
		ArrayList<Integer> tNs = SQL.getTickets(pageNumber, "User",
				player.getName());
